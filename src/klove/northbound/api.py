...
def create_api(registry: PrinterRegistry, authenticator: BearerAuthenticator) -> web.Application:
...
def require_scope(scope: str, handler: Handler) -> Handler:
    ...
    async def guarded(request: web.Request) -> web.StreamResponse:
        principal = request.app[authenticator_key].authenticate(
...
async def list_printers(request: web.Request) -> web.Response:
    ...
    return web.json_response(
        {"printers": [snapshot.model_dump(mode="json") for snapshot in snapshots]}
    )
