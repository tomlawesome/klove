...
class CompatibilitySessionRegistry:
    ...
    def fence_all(self) -> None:
        """Revoke every session when durable lifecycle evidence is uncertain."""
        sessions = tuple(
            session for printer_sessions in self._sessions.values() for session in printer_sessions
        )
    ...
    def reconcile_committed(self, record: RegisteredPrinter) -> None:
        ...
        if previous is not None:
            if record.revision < previous.revision:
                raise ValueError("committed printer revision regressed")
