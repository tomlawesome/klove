...
async def test_late_stale_session_registration_closes_before_mqtt_admission_response(
    tmp_path: Path,
) -> None:
    record = printer(
...
async def test_current_session_registration_unregisters_after_mqtt_cleanup(tmp_path: Path) -> None:
    record = printer(
        printer_uuid=PRINCIPAL.printer_uuid,
        safety_profiles=(safety_profile(printer_uuid=PRINCIPAL.printer_uuid),),
    )
    ...
    target = await run_session(server, connect(serial=record.proxy_serial) + frame(0xE0))
